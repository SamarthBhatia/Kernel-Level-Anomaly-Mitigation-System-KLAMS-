...
function_socket_filter = network_filter.load_func("socket_filter", BPF.SOCKET_FILTER)
BPF.attach_raw_socket(function_socket_filter, "")

try:
    function_cgroup_filter = network_filter.load_func("cgroup_skb_egress", BPF.CGROUP_SKB)
    # Attach to root cgroup (requires privileges)
    network_filter.attach_func(function_cgroup_filter, "/sys/fs/cgroup", BPF.CGROUP_INET_EGRESS)
    print("✅ Cgroup network filter attached")
except Exception as e:
    print(f"⚠️ Cgroup filter attachment failed (expected on some systems): {e}")

print("✅ Network filter active - eBPF will now block traffic from flagged PIDs")
